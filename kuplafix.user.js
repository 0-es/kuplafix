...
// @updateURL    https://raw.githubusercontent.com/0-es/kuplafix/main/kuplafix.user.js
// @downloadURL  https://raw.githubusercontent.com/0-es/kuplafix/main/kuplafix.user.js
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  // The bookmarklet sets this before injecting us.  Consume it immediately so it
  // cannot accidentally affect a later script injection on the same page.
  const IS_BOOKMARKLET_BOOT = window.__KUPLAFIX_BOOKMARKLET_BOOT__ === true;
  try { delete window.__KUPLAFIX_BOOKMARKLET_BOOT__; } catch (_) {
    window.__KUPLAFIX_BOOKMARKLET_BOOT__ = undefined;
  }
  ...
  window[GLOBAL_SINGLETON_KEY] = {
    version: SCRIPT_VERSION,
    startedAt: Date.now(),
    bookmarkletBoot: IS_BOOKMARKLET_BOOT,
  };
  ...
  const DOM = {
    ...
    _bookmarkletRestarted: false,

    isGameDocument(docRef) {
      try {
        const href = docRef?.defaultView?.location?.href || docRef?.URL || '';
    ...
    _emitNitroIframeDoc(iframeDoc) {
      if (!iframeDoc || this._nitroLifecycleSuspended || !this.isGameDocument(iframeDoc)) return;
      ...
      this._nitroIframeCallbacks.forEach((cb) => {
        try {
          ...
          log.error('Nitro iframe lifecycle callback failed:', error);
        }
      });
    },
    ...
    getIframeDoc() {
      try {
        if (this._nitroLifecycleSuspended) return null;
        const iframe = this.getNitroIframe();
        const iframeDoc = iframe?.contentDocument || null;
        return this.isGameDocument(iframeDoc) ? iframeDoc : null;
      } catch {
        return null;
      }
    },

    releaseNitroLifecycle() {
      this._nitroLifecycleSuspended = false;
      this._nitroLastIframeDoc = null;
      this._bindNitroIframe();
    },

    waitForIframeLoad(iframe, predicate, timeout = 8000) {
      return new Promise((resolve, reject) => {
        ...
        const onLoad = () => {
          try {
            ...
            if (!predicate || predicate(doc)) finish(null, doc);
          } catch (_) { /* wait for the next document transition */ }
        };
        const timer = setTimeout(() => finish(new Error('Nitro iframe load timed out')), timeout);
        iframe.addEventListener('load', onLoad);
        onLoad();
      });
    },

    async restartNitroIframeForBookmarklet(installHook) {
      if (this._bookmarkletRestarted) {
        log.warn('Bookmarklet Nitro restart was already requested; ignoring duplicate request.');
        return false;
      }
      this._bookmarkletRestarted = true;

      let iframe = this.getNitroIframe();
      if (!iframe) {
        log.info('Bookmarklet: waiting for the Nitro iframe to appear.');
        iframe = await this.waitFor('#nitro', 8000);
      }
      if (!iframe) throw new Error('Nitro iframe (#nitro) was not found.');

      const source = new URL(iframe.getAttribute('src') || iframe.src, location.href).href;
      if (new URL(source).origin !== location.origin) {
        throw new Error(`Nitro iframe is not same-origin: ${source}`);
      }

      // Keep the Nitro URL intact. A same-origin about:blank document cannot be
      // rewritten to an arbitrary client URL with History API, which previously
      // left the player on a blank frame when replaceState threw.
      log.info('Bookmarklet: restarting Nitro with its original complete SSO URL.', source);
      // Do not accept the currently loaded client: wait for the document created
      // by the navigation below. Otherwise waitForIframeLoad can resolve before
      // the iframe has actually started its restart.
      const previousDocument = iframe.contentDocument;
      const clientLoad = this.waitForIframeLoad(
        iframe,
        (doc) => doc !== previousDocument && this.isGameDocument(doc)
      );
      try {
        iframe.src = source;
        const freshDoc = await clientLoad;
        const freshWindow = freshDoc?.defaultView || iframe.contentWindow;
        if (freshWindow) installHook(freshWindow);
        this.releaseNitroLifecycle();
        log.info('Bookmarklet: Nitro client restarted safely. Packet interception is deterministic only in document-start userscript mode.');
        return true;
      } catch (error) {
        // The iframe navigation is already in progress; never blank it or show a
        // blocking popup. Release lifecycle listeners so the native client can
        // finish loading normally.
        this.releaseNitroLifecycle();
        log.error('Bookmarklet Nitro restart did not confirm a load in time; allowing the native client to continue.', error);
        return false;
      }
    },
  ...
  const Styles = {
    ...
    injectBaseStyles() {
      const styleContent = `
        ...
        .kuplafix-chat-history-actions {
          display: flex;
          align-items: center;
          gap: 5px;
        }

        .kuplafix-chat-history-popout,
        ...
        .kuplafix-chat-history-popout:hover,
  ...
  const UI = {
    ...
    openMenu() {
      ...
      if (pendingUpdate && UpdateChecker.isNewerVersion(pendingUpdate, SCRIPT_VERSION)) {
        subtitleHtml = `
          <a href="${UpdateChecker.RAW_SCRIPT_URL}" target="_blank" rel="noopener" title="Avaa uusin .user.js ScriptCatia varten" style="
            color: #7cb1c8;
          ...
          " onmouseover="this.style.background='rgba(124, 177, 200, 0.1)'; this.style.border='1px solid #7cb1c8';"
            ...
            kuplafix ${SCRIPT_VERSION} → <span style="color: #4ade80;">${pendingUpdate}</span> · asenna uusin script
          </a>
        `;
      }
      ...
      const features = {
        ...
        'gif-blocker': {
          ...
          settingsBtn: `<button class="kuplafix-btn-secondary" id="kuplafix-options-gif-blocker" title="Hallitse estoja">Hallitse</button>`,
          content: `
  ...
  const UpdateChecker = {
    ...
    async checkForUpdates() {
      ...
      try {
        const url = this.RAW_SCRIPT_URL + '?t=' + now;
        log.info('Fetching latest version from the repository: ' + url);
        ...
        return new Promise((resolve, reject) => {
          GM_xmlhttpRequest({
            ...
            onload: (response) => {
              log.info('Repository update response received, status: ' + response.status);
    ...
    parseAndNotifyUpdate(html) {
      try {
        ...
        // The raw userscript carries its version in the UserScript metadata block.
        const versionMatch = html.match(/Version:\s*([0-9.]+)/i) ||
          html.match(/@version\s+([0-9.]+)/);

        if (!versionMatch || !versionMatch[1]) {
          log.warn('Could not extract a version from the repository response.');
    ...
    updateMenuHeaderLink(latestVersion, docRef = document) {
      ...
      subtitle.innerHTML = `
        <a href="${this.RAW_SCRIPT_URL}" target="_blank" rel="noopener" title="Avaa uusin .user.js ScriptCatia varten" style="
          color: #7cb1c8;
          text-decoration: none;
          display: inline-block;
          padding: 4px 8px;
        ...
        " onmouseover="this.style.background='rgba(124, 177, 200, 0.1)'; this.style.border='1px solid #7cb1c8';"
           onmouseout="this.style.background='transparent'; this.style.border='1px solid transparent';">
          kuplafix ${currentVersion} → <span style="color: #4ade80;">${latestVersion}</span> · asenna uusin script
        </a>
      `;
  ...
  const ChatHistoryUI = {
    ...
    createWindow() {
      ...
      header.innerHTML = `
        ...
        <div class="kuplafix-chat-history-actions">
          <button type="button" class="kuplafix-chat-history-popout" title="Avaa erilliseen chat-ikkunaan" aria-label="Avaa erilliseen chat-ikkunaan">↗</button>
          <button type="button" class="kuplafix-chat-history-close" title="Sulje" aria-label="Sulje">✕</button>
        </div>
      `;
      ...
      header.querySelector('.kuplafix-chat-history-popout').addEventListener('click', () => this.openPopout());
    ...
    filterMessages(query, hideSystem, hideBots, hideWired) {
      this.applyFilters(this.content, query, hideSystem, hideBots, hideWired);
      this.applyFilters(this.popoutContent, query, hideSystem, hideBots, hideWired);
      this.syncPopoutFilters(query, hideSystem, hideBots, hideWired);
    },

    applyFilters(content, query, hideSystem, hideBots, hideWired) {
      if (!content) return;
      this.applyFiltersToWrappers(Array.from(content.children), query, hideSystem, hideBots, hideWired);
    },

    applyFiltersToWrappers(wrappers, query, hideSystem, hideBots, hideWired) {
    ...
    getFilterState() {
      ...
      return {
        query: source?.querySelector('.kuplafix-chat-history-search-input')?.value || '',
        hideSystem: !!source?.querySelector('#kf-hide-system')?.checked,
        hideBots: !!source?.querySelector('#kf-hide-bots')?.checked,
        hideWired: !!source?.querySelector('#kf-hide-wired')?.checked,
      };
    },

    syncPopoutFilters(query, hideSystem, hideBots, hideWired) {
      const doc = this.popoutWindow?.document;
      if (!doc) return;
      const search = doc.querySelector('.kuplafix-chat-history-search-input');
      const system = doc.querySelector('#kf-hide-system');
      const bots = doc.querySelector('#kf-hide-bots');
      const wired = doc.querySelector('#kf-hide-wired');
      if (search) search.value = query;
      if (system) system.checked = hideSystem;
      if (bots) bots.checked = hideBots;
      if (wired) wired.checked = hideWired;
    },
    ...
    openPopout() {
      ...
      // Keep the game DOM where it is. The pop-out is an independent, lightweight
      // view fed from the chat cache, so closing or updating it cannot affect Nitro.
      const popupHost = this.targetDoc?.defaultView || window;
      const popup = popupHost.open('', 'kuplafix-chat-history', 'popup=yes,width=460,height=640,resizable=yes,scrollbars=no');
      if (!popup) {
        UI.showToast('Selain esti chat-ikkunan. Salli ponnahdusikkunat Kuplahotellille ja yritä uudelleen.');
        return;
      }

      popup.document.open();
      popup.document.write(`<!doctype html>
        <html lang="fi"><head><meta charset="utf-8"><title>KuplaFix · Viestihistoria</title>
        <style>
          * { box-sizing: border-box; }
          html, body { width: 100%; height: 100%; overflow: hidden; }
          body { margin: 0; background: #14191e; }
          .kuplafix-chat-history-popout-root { height: 100%; display: flex; flex-direction: column; background: rgba(20, 25, 30, .98); }
          .kuplafix-chat-history-header { flex: 0 0 33px; }
          .kuplafix-chat-history-content { min-height: 0; padding-bottom: 14px; }
        </style></head><body>
          <div class="kuplafix-chat-history-popout-root">
            <div class="kuplafix-chat-history-header">
              <div class="kuplafix-chat-history-title">Viestihistoria</div>
              <div class="kuplafix-chat-history-actions"><button class="kuplafix-chat-history-close" id="close" type="button" title="Sulje">✕</button></div>
            </div>
            <div class="kuplafix-chat-history-search-bar">
              <input type="text" class="kuplafix-chat-history-search-input" placeholder="Etsi viestejä..." />
              <div style="display: flex; gap: 10px; margin-top: 6px; font-size: 11px; color: #ccc; flex-wrap: wrap;">
                <label style="display: flex; align-items: center; gap: 4px; cursor: pointer;"><input type="checkbox" id="kf-hide-system" /> Piilota järjestelmäviestit</label>
                <label style="display: flex; align-items: center; gap: 4px; cursor: pointer;"><input type="checkbox" id="kf-hide-bots" /> Piilota bottiviestit</label>
                <label style="display: flex; align-items: center; gap: 4px; cursor: pointer;"><input type="checkbox" id="kf-hide-wired" /> Piilota WIRED-viestit</label>
              </div>
            </div>
            <main id="messages" class="kuplafix-chat-history-content nitro-chat-history" aria-live="polite"></main>
          </div>
        </body></html>`);
      ...
      const popoutSearch = popup.document.querySelector('.kuplafix-chat-history-search-input');
      const popoutSystem = popup.document.querySelector('#kf-hide-system');
      const popoutBots = popup.document.querySelector('#kf-hide-bots');
      const popoutWired = popup.document.querySelector('#kf-hide-wired');
      const syncToGame = () => {
        const gameSearch = this.window?.querySelector('.kuplafix-chat-history-search-input');
        const gameSystem = this.window?.querySelector('#kf-hide-system');
        const gameBots = this.window?.querySelector('#kf-hide-bots');
        const gameWired = this.window?.querySelector('#kf-hide-wired');
        if (!gameSearch || !gameSystem || !gameBots || !gameWired) return;
        gameSearch.value = popoutSearch.value;
        gameSystem.checked = popoutSystem.checked;
        gameBots.checked = popoutBots.checked;
        gameWired.checked = popoutWired.checked;
        this.filterMessages(gameSearch.value, gameSystem.checked, gameBots.checked, gameWired.checked);
      };
      popoutSearch.addEventListener('input', syncToGame);
      popoutSystem.addEventListener('change', syncToGame);
      popoutBots.addEventListener('change', syncToGame);
      popoutWired.addEventListener('change', syncToGame);
      popup.addEventListener('beforeunload', () => {
        if (this.popoutWindow === popup) {
          this.popoutWindow = null;
          this.popoutContent = null;
        }
      }, { once: true });
    ...
    refreshPopout() {
      if (!this.popoutWindow || this.popoutWindow.closed || !this.popoutContent) return;

      const fragment = this.popoutWindow.document.createDocumentFragment();
      let previousTime = null;
      for (const msg of ChatHistoryCache.cache) {
        const messageTime = msg.fullDateTime ? new Date(msg.fullDateTime).getTime() : (msg.cachedAt || Date.now());
        if (previousTime && messageTime - previousTime >= 30 * 60 * 1000) {
          fragment.appendChild(this.createSessionDivider(this.popoutWindow.document, new Date(messageTime).toISOString()));
        }
        fragment.appendChild(this.createPopoutMessage(msg));
        previousTime = messageTime;
      }
      this.popoutContent.replaceChildren(fragment);
      const filters = this.getFilterState();
      this.applyFilters(this.popoutContent, filters.query, filters.hideSystem, filters.hideBots, filters.hideWired);
      this.popoutContent.scrollTop = this.popoutContent.scrollHeight;
    },

    createPopoutMessage(msg) {
      return this.renderMessage(msg, this.popoutWindow.document, false);
    },

    appendPopoutMessage(msg) {
      if (!this.popoutWindow || this.popoutWindow.closed || !this.popoutContent) return;

      const wasNearBottom = this.popoutContent.scrollHeight - this.popoutContent.scrollTop - this.popoutContent.clientHeight < 100;
      const cache = ChatHistoryCache.cache;
      const previous = cache[cache.length - 2];
      const currentTime = msg.fullDateTime ? new Date(msg.fullDateTime).getTime() : (msg.cachedAt || Date.now());
      const previousTime = previous
        ? (previous.fullDateTime ? new Date(previous.fullDateTime).getTime() : (previous.cachedAt || currentTime))
        : null;
      if (previousTime && currentTime - previousTime >= 30 * 60 * 1000) {
        this.popoutContent.appendChild(this.createSessionDivider(this.popoutWindow.document, new Date(currentTime).toISOString()));
      }
      const message = this.createPopoutMessage(msg);
      this.popoutContent.appendChild(message);
      const filters = this.getFilterState();
      this.applyFiltersToWrappers([message], filters.query, filters.hideSystem, filters.hideBots, filters.hideWired);
      if (wasNearBottom) this.popoutContent.scrollTop = this.popoutContent.scrollHeight;
    },
    ...
    showSessionDivider(dateTime) {
      ...
      this.content.appendChild(this.createSessionDivider(this.targetDoc || document, dateTime));
    },

    createSessionDivider(doc, dateTime) {
    ...
    addMessage(msg) {
      ...
      this.appendPopoutMessage(msg);
    },

    renderMessage(msg, docRef = this.targetDoc || document, enableTimestampPopup = true) {
      ...
      if (enableTimestampPopup && (msg.fullDateTime || msg.cachedAt)) {
  ...
  const PacketManager = {
    ...
    // Helper: Get structure from header entry
    _getStruct(entry) { return Array.isArray(entry) ? entry[1] : null; },
    ...
    _buildStructMap() {
      ...
      this._structMap = {};
      for (const [name, entry] of Object.entries(this.HEADERS.OUTGOING)) {
        const struct = this._getStruct(entry);
        if (struct) this._structMap[this._getId(entry)] = struct;
      }
      for (const [name, entry] of Object.entries(this.HEADERS.INCOMING)) {
        const struct = this._getStruct(entry);
        if (struct) this._structMap[this._getId(entry)] = struct;
      }
    },
    ...
    HEADERS: {
      OUTGOING: {
        ...
        "CATALOG_PURCHASE": [3492, 'IISI'],      // pageId, itemId, extraData, count
        "CATALOG_PURCHASE_GIFT": 1411,
        "GET_PRODUCT_OFFER": [2594, 'I'],        // spriteId
        "CLIENT_LATENCY": 295,
        ...
        "TRADE_UNACCEPT": 1444,
        "UNIT_ACTION": 2456,
        "UNIT_CHAT": [1314, 'SII'],              // message, styleId, trackingId
        "UNIT_CHAT_SHOUT": [2085, 'SI'],         // message, styleId
        "UNIT_CHAT_WHISPER": [1543, 'SSI'],      // target, message, styleId
        "UNIT_DANCE": 2080,
    ...
    installHook(win) {
      if (!win || this._hookedWindows.has(win)) return false;

      const OriginalWebSocket = win.WebSocket;
      if (typeof OriginalWebSocket !== 'function' || !OriginalWebSocket.prototype?.send) {
        log.warn('PacketManager: WebSocket is not available on a Nitro window yet.');
        return false;
      }
      this._hookedWindows.add(win);
      log.info('PacketManager: installed WebSocket hook on', win === window ? 'top window' : 'fresh Nitro window');
      ...
      const WebSocketProxy = function (...args) {
        if (!new.target) {
        ...
        if (urlText.includes('hanarchy.net')) {
          log.info('PacketManager: new WebSocket called with', urlText);
        }

        // Reflect.construct preserves the native constructor's argument handling
        // (including a one-argument call and protocol arrays).
        const ws = Reflect.construct(OriginalWebSocket, args, OriginalWebSocket);
        ...
        if (urlText.includes('hanarchy.net')) {
          log.info('PacketManager: captured first game WebSocket on fresh Nitro window', urlText);
          self.socket = ws;
      ...
      try { Object.setPrototypeOf(WebSocketProxy, OriginalWebSocket); } catch (_) { }

      // Apply the hijack
      try {
        win.WebSocket = WebSocketProxy;
      } catch (e) {
        this._hookedWindows.delete(win);
    ...
    // Compatibility alias for integrations that used the old internal name.
    hijackWebSocket(win) {
      return this.installHook(win);
    },
    ...
    processBuffer(direction, buffer) {
      if (buffer.byteLength >= 6) {
        ...
        this._dispatchToListeners(direction, header, buffer);
    ...
    // Type code mapping for compact format (I=Int, S=String, H=Short, B=Byte, O=Boolean)
    _typeMap: { I: 'Int', S: 'String', H: 'Short', B: 'Byte', O: 'Boolean' },

    // Improved Parser for Import - Now Protocol-Aware
    parsePacket(buffer, header = null) {
      const args = [];
      ...
      // Extract header from buffer if not provided
      if (header === null) {
        header = view.getInt16(4);
      }
      ...
      // ═══════════════════════════════════════════════════════════════════════
      // PROTOCOL-AWARE PARSING: Use exact structure if known (from HEADERS)
      // ═══════════════════════════════════════════════════════════════════════
      this._buildStructMap(); // Build map on first use
      const structureStr = this._structMap[header];
      if (structureStr) {
        for (const code of structureStr) {
          const remaining = buffer.byteLength - offset;
          if (remaining <= 0) break;
          const type = this._typeMap[code];
          if (!type) continue;

          switch (type) {
            case 'Int':
              if (remaining >= 4) {
                args.push({ type: 'Int', value: view.getInt32(offset), hex: getHex(offset, 4) });
                offset += 4;
              }
              break;
            ...
            case 'Short':
              if (remaining >= 2) {
                args.push({ type: 'Short', value: view.getInt16(offset), hex: getHex(offset, 2) });
                offset += 2;
              }
              break;
            ...
            case 'Byte':
              if (remaining >= 1) {
                args.push({ type: 'Byte', value: new Uint8Array(buffer)[offset], hex: getHex(offset, 1) });
                offset += 1;
              }
              break;
            ...
            case 'Boolean':
              if (remaining >= 1) {
                args.push({ type: 'Boolean', value: new Uint8Array(buffer)[offset] !== 0, hex: getHex(offset, 1) });
                offset += 1;
              }
              break;
            ...
            case 'String':
              if (remaining >= 2) {
                ...
                if (strLen >= 0 && strLen <= remaining - 2) {
                  const strBytes = new Uint8Array(buffer, offset + 2, strLen);
                  let strVal = '';
                  try { strVal = new TextDecoder().decode(strBytes); } catch (e) { }
                  args.push({ type: 'String', value: strVal, hex: getHex(offset, 2 + strLen) });
                  offset += 2 + strLen;
                }
              }
              break;
      ...
      // ═══════════════════════════════════════════════════════════════════════
      // HEURISTIC PARSING: Fallback for unknown packets
      // ═══════════════════════════════════════════════════════════════════════
      ...
      while (offset < buffer.byteLength) {
        ...
        // 1. Try String (Heuristic: length > 0 and printable, OR empty string)
        if (remaining >= 2) {
          const strLen = view.getInt16(offset);

          // Check for valid string candidates
          let isString = false;
          let strVal = '';

          if (strLen === 0) {
            // Empty string? 
            // Prefer empty string if:
            // a) remaining == 2 (packet ends)
            // b) remaining >= 6 AND the next 4 bytes don't look like a valid small Int 
            //    OR the next 2 bytes look like a valid printable string length.

            if (remaining === 2) {
              isString = true;
            } else if (remaining >= 4) {
              // Heuristic: If parsing as Int32 gives a small positive number, 
              // it's VERY likely an ID or counter, not an empty string followed by junk.
              const intVal = view.getInt32(offset);
              if (intVal < 0 || intVal > 1000000) {
                isString = true;
              }
            }
          } else if (strLen > 0 && strLen <= remaining - 2 && strLen < 2048) {
            // Check printability
            const strBytes = new Uint8Array(buffer, offset + 2, strLen);
            let printable = true;
            for (let i = 0; i < strLen; i++) {
              const c = strBytes[i];
              // Allow some basic control chars but mostly printable
            ...
            if (printable) {
              try {
                strVal = new TextDecoder().decode(strBytes);
                // If it contains the replacement character (invalid UTF-8 sequence usually), treat as binary
                if (strVal.includes('\uFFFD')) {
                  isString = false;
                } else {
                  isString = true;
      ...
      for (const arg of args) {
        ...
        if (typeof arg === 'object' && arg.type) {
          ...
          switch (arg.type) {
            ...
            case 'Boolean': writeByte(arg.value ? 1 : 0); break;
    ...
    // ─────────────────────────────────────────────────────────────────
    // Packet Listener API (for external scripts like Roomilus)
    // ─────────────────────────────────────────────────────────────────

    /**
     * Register a listener for incoming packets (server → client)
     * @param {number|string} header - Packet header ID or '*' for all packets
     * @param {function} callback - Called with (header, buffer, parsedArgs)
    ...
    /**
     * Register a listener for outgoing packets (client → server)
     * @param {number|string} header - Packet header ID or '*' for all packets
     * @param {function} callback - Called with (header, buffer, parsedArgs)
     * @returns {function} Unsubscribe function
     */
    onOutgoing(header, callback) {
      return this._addListener('outgoing', header, callback);
    },

    /**
     * Remove a specific listener
     * @param {string} direction - 'incoming' or 'outgoing'
     * @param {number|string} header - Packet header ID or '*'
     * @param {function} callback - The callback to remove
     */
    offPacket(direction, header, callback) {
      const map = this._listeners[direction];
      if (!map) return;
    ...
    _addListener(direction, header, callback) {
      const map = this._listeners[direction];
      const key = String(header);
      ...
      log.debug(`PacketManager: Listener added for ${direction} [${header}]`);

      // Return unsubscribe function
      return () => this.offPacket(direction, header, callback);
    },

    _dispatchToListeners(direction, header, buffer) {
      const dir = direction === 'IN' ? 'incoming' : 'outgoing';
      const map = this._listeners[dir];
      if (!map || map.size === 0) return;

      // Parse args once for all listeners
      let parsedArgs = null;
      const getParsedArgs = () => {
        if (parsedArgs === null) {
          try {
            parsedArgs = this.parsePacket(buffer);
          } catch (e) {
            parsedArgs = [];
          }
        }
      ...
      // Dispatch to specific header listeners
      const specificListeners = map.get(String(header));
      if (specificListeners) {
        specificListeners.forEach(cb => {
          try {
            cb(header, buffer, getParsedArgs());
          } catch (e) {
            console.error(`[PacketManager] Listener error for ${dir}[${header}]:`, e);
          }
        });
      }

      // Dispatch to wildcard listeners
      const wildcardListeners = map.get('*');
      if (wildcardListeners) {
        wildcardListeners.forEach(cb => {
          try {
            cb(header, buffer, getParsedArgs());
          } catch (e) {
            console.error(`[PacketManager] Wildcard listener error for ${dir}:`, e);
          }
        });
      }
    },
    ...
    getListenerCount() {
      let count = 0;
      for (const map of Object.values(this._listeners)) {
  ...
  const PacketBuilder = {
    ...
    createLogLine(p, direction = 'OUT') {
      ...
      const headerMap = direction === 'OUT' ? PacketManager.HEADERS.OUTGOING : PacketManager.HEADERS.INCOMING;
      for (const [name, entry] of Object.entries(headerMap)) {
        if (PacketManager._getId(entry) === p.header) {
          headerName = name;
          break;
        }
      }
  ...
  // ─────────────────────────────────────────────────────────────────
  // WardrobeFreezer removed — legacy wardrobe optimization feature was deleted and is no longer loaded.
  ...
  let globalModulesStarted = false;

  async function startGlobalModules() {
    if (globalModulesStarted) return;
    globalModulesStarted = true;
  ...
  async function initializeGlobalModules() {
    log.info(`Initializing global modules v${SCRIPT_VERSION}...`);
    await startGlobalModules();
    log.info('✓ Global modules initialized');
  }

  // Core state and hooks are set up before UI modules. PacketManager has its
  // own one-time guard because it must be available before a bookmarklet client
  // restart starts executing Nitro's module scripts.
  config.load();
  ...
  if (IS_BOOKMARKLET_BOOT) {
    ...
    // Start controllers in the background. DOM suspension prevents any of them
    // from binding to the old client; late registrations receive the fresh doc.
    initializeGlobalModules().catch((err) => log.error('Global initialization failed:', err));
    DOM.restartNitroIframeForBookmarklet((freshWindow) => PacketManager.installHook(freshWindow))
      .catch((err) => {
        DOM.releaseNitroLifecycle();
        log.error('Bookmarklet Nitro restart setup failed; the current game client was left untouched.', err);
      });
